...
public class GoogleMapsActivity extends /*FragmentActivity*/AppCompatActivity implements BuoyInputDialog.BuoyInputDialogListener{
    ...
    private void AddRaceCourse() {
        ...
        if (raceCourse.getMarks()!=null){ //TODO : Check why being removed twice!
            for(AviObject ao: raceCourse.getMarks().marks){
        ...
        raceCourse.setBoatVMGUpwind(getVMGUpwind(boatClass,(int) Float.parseFloat(SP.getString("windSpd", "14"))));
        raceCourse.setBoatVMGRun(getVMGRun(boatClass, (int) Float.parseFloat(SP.getString("windSpd", "14"))));
        raceCourse.setBoatVMGReach(getVMGReach(boatClass, (int) Float.parseFloat(SP.getString("windSpd", "14"))));
        raceCourse.setSignalBoatLoc(GeoUtils.toAviLocation(iGeo.getLoc()));
        raceCourse.setWindDir(Integer.parseInt(SP.getString("windDir", "0"))); //TODO putout to special function to get wind dir
        raceCourse.setWindSpeed((int) Float.parseFloat(SP.getString("windSpd", "14")));
        raceCourse.setBoatLength(getBoatLength(boatClass));
        raceCourse.setGoalTime(Integer.parseInt(SP.getString("goalTime", "50")));
        raceCourse.setRaceCourseType(raceCourse.getRaceCourseType(SP.getString("rcType","Windward-leeward"))); //TODO redundant
        raceCourse.setNumOfBoats(Integer.parseInt(SP.getString("numOfBoats", "25")));
        raceCourse.calculateCourse(raceCourse.getRaceCourseType(SP.getString("rcType","Windward-leeward")));
        Marks marks = raceCourse.getMarks();
